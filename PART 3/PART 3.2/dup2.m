...
for kval =  1:3
	% frame sync
K = 8;
pilot_length = [10 20 40]*number_of_bits;
pilot = randi(2, pilot_length(kval),1)-1;
encoded_pilot = mapping(pilot, number_of_bits, modulation);
pilot_int = pilot_length(kval)+40*number_of_bits;
for lolol = 1:len_rep 
	fprintf(""+kval+" "+lolol+"\n")
...
if (not(mod(bitstream_length/(pilot_int-pilot_length(kval)),1) == 0))
...
if (not(mod(pilot_length(kval)/number_of_bits,1) == 0))
...
for i = 1:pilot_int:bitstream_length+pilot_length(kval)*ceil(bitstream_length/(pilot_int-pilot_length(kval)))
...
%fprintf("("+indddex+")\n")
[argmaxn, ~] = toa_est(filtered_signals_receiver(1:pilot_int_enc,indddex),encoded_pilot,K,1/2e6);
...
for start = 1:pilot_int_enc:length(filtered_signals_receiver)-pilot_int_enc
	...
	[arg_now, cfo_rec_now] = toa_est(filtered_signals_receiver(start:start+pilot_int_enc, indddex), encoded_pilot, K, 1/2e6);
...
title("CFO error with varying pilot length N, K = 8")
xlabel("SNR [dB]")
...
title("Time error with varying pilot length N, K = 8")
xlabel("SNR [dB]")
